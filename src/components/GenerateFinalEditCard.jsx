...
const FinalEditContentCard = ({ generatedData }) => {
  const { node_displayed, edit_certain_node, data_global, set_node_displayed } =
  ...
  const handleConfirm = () => {
    const updatedMilestones1 = node_displayed.children.filter(
      (_, index) => milestoneStatusOrig[index]
    );
    const updatedMilestones2 = generatedData.children.filter(
      (_, index) => milestoneStatusGene[index]
    );
  ...
  return (
    <div style={styles.container}>
      ...
      >
        ...
        <Button type="primary" onClick={allGenerated}>
          All Generated
        </Button>
      </div>
